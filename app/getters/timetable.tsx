...
function getTimetable(): Promise<Period[][] | null> {
    return new Promise((resolve, reject) => {
        fetchHTMLResource("/timetable").then((document: HTMLElement) => {
            var getTimetableHeaderName = (th: HTMLElement) =>
                th.childNodes[0].textContent.trim();

            console.log("timetable.tsx:22 says:", "looking for timetable");

            if (document.querySelector(".empty-state")) return resolve(null) // if no timetable found

            console.log("timetable.tsx:26 says: found");

            var data: any = {};
            var times: string[] = [];
            var timeNames: string[][] = [];
            var timetable: HTMLElement = document.querySelector(".timetable");
            ...
            timetable.querySelectorAll(".timetable-subject").forEach((el: HTMLElement) => {
...
function getDay(day: number, now: any): Promise<Period[] | null> {
    return new Promise((resolve, reject) => {
        getTimetable().then((timetable) => {
            console.log("timetable.tsx:155 says: a");
            if (timetable == null) {
                return resolve(null)
            }
            console.log("timetable.tsx:159 says:", "b");
...
function getDayAndFull(day: number, now: any): Promise<[undefined, any, Period[][]] | null> {
    //TODO: fix resolve to be better
    return new Promise((resolve, reject) => {
        getTimetable().then((longTimetable) => {
            if (longTimetable == null) {
                return resolve(null)
            }
...
function getNowOnwards(): Promise<Period[] | null> {
    return new Promise((resolve, reject) => {
        let now = new Date();
